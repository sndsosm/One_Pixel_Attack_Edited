...
from cifar10_models.vgg import vgg13_bn,vgg11_bn,vgg16_bn,vgg19_bn
...
from torch.autogsrad import Variable
import torch as T
...
parser.add_argument('--model',default='resnet18', choices=model_defs.keys(), type=str)
...
def attack(img, label, net, target=None, pixels=1, maxiter=75, popsize=400, verbose=False):
  ...
  imshow(tv.utils.make_grid(attack_image))
...
def main():
  ...
  base = [model_defs[m](pretrained=True) for m in [args.model]]
  results = attack_all(net=base[0].to(device), loader=testloader, pixels=args.pixels, targeted=args.targeted, maxiter=args.maxiter, popsize=args.popsize, verbose=args.verbose)
